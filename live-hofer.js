(() => {
  ...
  const DATA_URL = "data/hofer.json";
  ...
  async function load(force = false) {
    ...
    loadPromise = fetch(url, { cache: force ? "reload" : "default" })
      .then(async response => {
        if (!response.ok) throw new Error(`HOFER-Daten nicht verfügbar (${response.status})`);

        const payload = await response.json();
        if (!payload || !Array.isArray(payload.products)) {
          throw new Error("Ungültiges HOFER-Datenformat.");
        }
  ...
  async function getObject(id) {
    if (!id) throw new Error("HOFER-Produkt-ID fehlt.");
    const payload = await load(false);
    const item = lookup?.get(String(id));
    if (!item) throw new Error("HOFER-Produkt im aktuellen Datenstand nicht gefunden.");
