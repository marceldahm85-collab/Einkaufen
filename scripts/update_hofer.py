...
def normalize_item(item: dict):
    if str(item.get("store") or "").lower() != "hofer":
        return None
...
def main() -> int:
    ...
    try:
        ...
        if len(products) < MIN_EXPECTED_PRODUCTS:
            raise RuntimeError(
                f"Unplausibel wenige HOFER-Produkte ({len(products)}). "
        ...
        payload = {
            ...
            "importerVersion": 2,
            "store": "hofer",
            "scope": "HOFER-Daten aus Heisse Preise",
            "provider": "heisse-preise.io",
        ...
        print(
            ...
            f"{skipped} HOFER-Einträge übersprungen."
        )
        return 0
        ...
        # solange die öffentliche HOFER-Datei noch leer ist.
        write_status(
            "error",
            updated_at,
            existing_count,
