...
def normalize_item(item: dict):
    if str(item.get("store") or "").lower() != "billa":
        return None
...
def main() -> int:
    ...
    try:
        ...
        if len(products) < MIN_EXPECTED_PRODUCTS:
            raise RuntimeError(
                f"Unplausibel wenige BILLA-Produkte ({len(products)}). "
        ...
        payload = {
            ...
            "importerVersion": 2,
            "store": "billa",
            "scope": "BILLA-Daten aus Heisse Preise",
            "provider": "heisse-preise.io",
        ...
        print(
            f"BILLA: {len(products)} Produkte importiert; "
            f"{skipped} BILLA-Einträge übersprungen."
        )
        return 0
        ...
        if existing_count >= MIN_EXPECTED_PRODUCTS:
            ...
            print(
                f"WARNUNG: BILLA konnte nicht aktualisiert werden. "
        ...
        # solange die öffentliche BILLA-Datei noch leer ist.
        write_status(
            "error",
            updated_at,
            existing_count,
